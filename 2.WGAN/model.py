...
class Discriminator(nn.Module):
    def __init__(self, img_c, features_d):
        super(Discriminator, self).__init__()
        self.disc = nn.Sequential(
            # Input : N x img_c x 64 x 64
            nn.Conv2d(
                img_c, features_d, kernel_size=4, stride=2, padding=1
            ), # 32 x 32
            nn.LeakyReLU(0.2),
...
class Generator(nn.Module):
    def __init__(self, z_dim, img_c, features_g):
        ...
        self.gen = nn.Sequential(
            ...
            nn.ConvTranspose2d(
                features_g*2, img_c, kernel_size=4, stride=2, padding=1
            ),
            nn.Tanh() # [-1, 1]
        )
